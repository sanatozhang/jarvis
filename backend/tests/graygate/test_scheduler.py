...
1. BJT 9 点整命中 → 触发一次 build_report + 发送（mock send_message）。
2. 同一天内多次 tick（9:00 和 9:01 都落在 hour==9）→ 只发一次（进程级幂等）。
3. 次日 9 点 → 再次触发（幂等状态按天重置）。
4. enabled=False / scheduler_enabled=False → 不调用 build_report。
5. feishu_enabled=False → 调用 build_report 但不调用 send_message。
6. GraygateReport(available=False) → 不调用 send_message，心跳仍然写
   （断言被调用且 summary 里记录了 available=false 的事实）。

全部 mock `get_graygate_settings` / `_now_bjt` / `build_report` / `send_message` /
...
def _report(available: bool = True, markdown: str = "# report") -> object:
...
async def test_fires_at_report_hour_and_sends():
    ...
    with patch.object(sched, "get_graygate_settings", return_value=_settings()), \
         ...
         patch.object(sched, "build_report", new=AsyncMock(return_value=_report())) as mock_build, \
         patch.object(sched, "send_message", new=AsyncMock(return_value=True)) as mock_send, \
    ...
    mock_send.assert_awaited_once_with(chat_id="oc_graygate", text="# report")
...
async def test_same_day_multiple_ticks_send_only_once():
    ...
    with patch.object(sched, "get_graygate_settings", return_value=_settings()), \
         patch.object(sched, "build_report", new=AsyncMock(return_value=_report())) as mock_build, \
         patch.object(sched, "send_message", new=AsyncMock(return_value=True)), \
...
async def test_enabled_false_skips_build_report():
    with patch.object(sched, "get_graygate_settings", return_value=_settings(enabled=False)), \
         ...
         patch.object(sched, "build_report", new=AsyncMock()) as mock_build, \
         patch.object(sched, "send_message", new=AsyncMock()) as mock_send, \
         patch.object(sched, "_write_heartbeat", new=AsyncMock()) as mock_hb:
        await sched._tick_once()
...
async def test_report_unavailable_skips_send_but_still_writes_heartbeat():
    """GraygateReport(available=False) → 不调用 send_message，心跳仍然写，
    summary 里记录 available=false 这个事实（不是报错，status 仍是 success）。"""
    with patch.object(sched, "get_graygate_settings", return_value=_settings()), \
         patch.object(sched, "_now_bjt", return_value=datetime(2026, 8, 19, 9, 0, 0, tzinfo=_BJT)), \
         patch.object(sched, "build_report", new=AsyncMock(return_value=_report(available=False, markdown=""))), \
         patch.object(sched, "send_message", new=AsyncMock()) as mock_send, \
         patch.object(sched, "_write_heartbeat", new=AsyncMock()) as mock_hb:
        await sched._tick_once()
...
async def test_send_failure_marks_degraded():
    ...
    with patch.object(sched, "get_graygate_settings", return_value=_settings()), \
         patch.object(sched, "_now_bjt", return_value=datetime(2026, 8, 19, 9, 0, 0, tzinfo=_BJT)), \
         patch.object(sched, "build_report", new=AsyncMock(return_value=_report())), \
         patch.object(sched, "send_message", new=AsyncMock(return_value=False)), \
         patch.object(sched, "_write_heartbeat", new=AsyncMock()) as mock_hb:
        await sched._tick_once()
...
async def test_build_report_exception_marks_failed():
    ...
    with patch.object(sched, "get_graygate_settings", return_value=_settings()), \
         patch.object(sched, "_now_bjt", return_value=datetime(2026, 8, 19, 9, 0, 0, tzinfo=_BJT)), \
         patch.object(sched, "build_report", new=AsyncMock(side_effect=RuntimeError("datadog boom"))), \
         patch.object(sched, "send_message", new=AsyncMock()) as mock_send, \
         patch.object(sched, "_write_heartbeat", new=AsyncMock()) as mock_hb:
        await sched._tick_once()
