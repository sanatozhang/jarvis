...
# ---------------------------------------------------------------------------
# "主" app 发送（历史上归属 Apollo 的机器人身份，见 FeishuSettings.app_id/app_secret）
#
# 2026-09-15 新增：graygate 的"主要版本变更通知"要求用这个身份发送，跟上面
# send_interactive_card()/send_message() 走的 IM 专属 app（FEISHU_IM_APP_ID）是
# 两个不同的机器人。物理拆仓前两边共用同一个"主" app；拆仓后 jarvis 保留了一份
# 凭证副本（FEISHU_APP_ID/FEISHU_APP_SECRET，见 backend/CLAUDE.md 基础设施说明），
# 仍然可用。token 独立缓存，不能跟 IM app 的 _im_token 共用——两个不同 app_id
# 换出来的 token 互不通用。
# ---------------------------------------------------------------------------
_main_app_token: Optional[str] = None
_main_app_token_expire: float = 0


async def _get_main_app_tenant_token() -> str:
    global _main_app_token, _main_app_token_expire
    now = time.monotonic()
    if _main_app_token and now < _main_app_token_expire:
        return _main_app_token
    ...
    async with httpx.AsyncClient(verify=False, timeout=30) as http:
        ...
        data = resp.json()
        _main_app_token = data["tenant_access_token"]
        _main_app_token_expire = now + data.get("expire", 7200) - 60
        return _main_app_token


async def send_interactive_card_as_main_app(
    chat_id: str = "",
    card: Dict[str, Any] = None,
    email: str = "",
) -> bool:
    """用"主" app（历史上 Apollo 的机器人身份）发送 interactive card。

    跟 send_interactive_card()（IM 专属 app）是两个不同的发送者身份——按业务需要
    选择哪个身份发送，不是互相替代关系。
    """
    import json as _json
    if not chat_id and not email:
    ...
    content = _json.dumps(card, ensure_ascii=False)
    try:
        token = await _get_main_app_tenant_token()
        url = "https://open.feishu.cn/open-apis/im/v1/messages"
        params = {"receive_id_type": "chat_id" if chat_id else "email"}
        body = {"receive_id": chat_id or email, "msg_type": "interactive", "content": content}
        async with httpx.AsyncClient(verify=False, timeout=30) as http:
            resp = await http.post(url, headers={"Authorization": f"Bearer {token}"}, params=params, json=body)
            result = resp.json()
            if result.get("code") != 0:
                raise RuntimeError(f"Feishu API error ({result.get('code')}): {result.get('msg', result)}")
        return True
    except Exception as e:
        logger.error("Failed to send interactive card via main app: %s", e)
        return False
