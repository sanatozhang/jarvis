...
- **没有 `env` tag**：4.0 native 的 RUM 事件不带任何 `env:*` 字段。测试环境和正式环境的
  区分机制是 Datadog RUM 的**独立 Application 注册**（不同 `application.id`/`application.name`）。
- Android：正式 `application.name=plaud_android`（`id=99b95c95-0967-407d-b064-2bad9d90fd06`），
  测试 `application.name=plaud_android_test`（`id=9843cec9-41b0-4e80-9515-c4bddb282643`）。
- iOS：正式 `application.name=plaud_ios`（`id=6eedd3c5-41e2-4426-8093-b238608ed6ce`），
  测试 `application.name=plaud_ios_test`（`id=8c3e549b-d83b-4dce-a4b2-445a63455fcd`）。
- **当前 crashguard 完全没有做这个过滤**，导致过去 14 天内 Android fatal crash issue
  （`@error.is_crash:true`）**100% 来自测试 App**（26/26），正式环境 0 个；拉长到 30 天，
  正式环境才有 14 个（测试环境同期 37 个）。iOS 30 天：正式 9 个 / 测试 31 个。
- `@application.name` 过滤和 `@application.id` 过滤结果完全一致（30天窗口下 android
  按 name/id 过滤都是 14），且是精确匹配（不会被 `plaud_android_test` 误命中），因此配置里
  优先用可读的 `@application.name`，UUID 只作注释里的备用核实手段。
- flutter（3.x）抽样 50 条 RUM 事件只有一个 `application.id`，没有 test/prod 拆分现象，
  确认 3.x 不需要同样的过滤。
- 已知的 Datadog Error Tracking issues search 返回上限约 100-102 条/次查询（项目既有记忆里
  `plaud-flutter 102(封顶)` 已经记录过），本次验证组合查询时一度看到"过滤前后总数相同"的
  假象就是这个封顶造成的，与过滤逻辑本身无关；已用远低于封顶量级的单服务查询隔离验证过滤
  逻辑本身正确。
...
```
(service:plaud-flutter OR (service:plaud_android AND @application.name:plaud_android) OR (service:plaud_ios AND @application.name:plaud_ios))
```

配置旁边加注释，记录：
- 这两个 `@application.name` 分别对应的 UUID（供 Datadog 上重命名后反查核实）；
- 核实用的 curl 命令（查 RUM events / error-tracking issues search，见上面"关键实测数据"）；
- **设计取舍**：用白名单精确匹配正式环境名字，不用黑名单排除 `*_test` 后缀——白名单失败
  即排除（以后出现命名不规范的新环境会被自动挡在外面，符合"只关注正式环境"的初衷），黑名单
  失败即放行（命名不按 `_test` 套路来的新环境会被误当成正式环境漏进来）。不要"优化"成黑名单。
...
3. **端到端从未真正跑通过一次**（过滤前的数据是测试噪声主导，真实正式环境 crash 极少）。
   现在已经有具体候选：过去 30 天正式环境 Android 有 14 个、iOS 有 9 个真实 crash issue。
...
- 回滚：A 部分如果发现正式环境过滤过严（比如漏掉了某个未预料到的正式环境 application 名字），
  直接把 `datadog_service_filter` 配置改回不带 `@application.name` 限制的版本即可，无需回滚
