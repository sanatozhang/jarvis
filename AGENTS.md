...
#### Codex CLI 登录（Docker 环境）

容器内 Codex 凭证通过 named volume `Codex-auth` 持久化，**首次部署后执行一次登录**：

```bash
docker compose exec -it backend Codex login
# 复制输出的 URL 到浏览器完成授权，登录信息永久保存在 Codex-auth volume 中
```

验证登录状态：

```bash
docker compose exec backend Codex config list
```
...
Backend (FastAPI + SQLAlchemy + SQLite)
  ↕ subprocess
Agents (Codex CLI / Codex CLI — external binaries)
```
...
7. **Run agent** — CLI subprocess (`Codex` or `codex`) writes JSON to `output/result.json`
...
**现象**：health check 显示 `Codex: not_installed`。

**根因**：Codex CLI 是 npm 包，Dockerfile 原来只装了 codex，未装 Codex。macOS 本机二进制是 Mach-O 格式，无法挂载到 Linux 容器直接使用。

**已修复**：`backend/Dockerfile` 加入：
```dockerfile
RUN npm install -g @anthropic-ai/Codex 2>/dev/null || echo "Codex install skipped (optional)"
```

### 4. Codex 登录态不持久

**现象**：容器重启后 Codex 需要重新登录。

**根因**：macOS 上 Codex 凭证存储在系统 Keychain，不在 `~/.Codex` 目录，无法通过挂载宿主机目录传递到容器。
...
```
首次部署执行一次 `docker compose exec -it backend Codex login` 即可。

### 5. Agent providers 为空（No enabled agent found）

**现象**：分析任务报错 `No enabled agent found. Tried 'Codex'. Available: []`。
