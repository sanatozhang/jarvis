...
- 设备时间：`device file:[TIMESTAMP]` 转换为日期（Unix 秒 → 日期时间）

### 步骤 3：计算偏移

```python
from datetime import datetime
app_time = datetime.fromisoformat("2025-12-01 13:43:21")
device_time = datetime.fromtimestamp(1695537220)  # device file 中的时间戳
drift = app_time - device_time
print(f"偏移：{drift.days} 天")
```
