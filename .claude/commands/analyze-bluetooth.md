...
### 2. 搜索到设备但无法连接（TokenNotMatch）— 最常见
...
- **原因**: APP端解绑时 token 未清除，导致新用户连接时云端与设备 token 不匹配
...
- **处理**: 引导用户登录正确账号
- 离线绑定走 `/device/bind-token`；正常云端绑定走 `/device/bind`
...
- **已知 case**: Note 升级到 131088 版本、Pin 升级到 131082 版本有概率出现蓝牙不广播
...
- **处理**: APP 没有主动断开 → 换货；APP 主动断开 → APP 问题
