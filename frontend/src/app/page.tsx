...
export default function HomePage() {
  ...
  const startAnalysis = async (issueId: string, isRetry = false, deepAnalysis = false, direction?: string) => {
    try {
      const task = await createTask(issueId, undefined, isRetry ? "" : (username || ""), direction || undefined, deepAnalysis);
  ...
  // Deep analysis launched from <AnalysisResultView> (its own dialog collected an optional direction).
  const handleDeepAnalysisWithDirection = (id: string, direction?: string) => {
    trackEvent("deep_analysis", { issue_id: id, has_direction: !!direction });
    startAnalysis(id, true, true, direction);
    setToast(t("深度分析已启动"));
  };
        ...
        <aside className="panel-slide-in fixed top-0 right-0 bottom-0 z-30 w-full lg:w-[35%] overflow-y-auto"
          style={{ background: "var(--j-panel)", borderLeft: `1px solid ${S.border}`, boxShadow: "-16px 0 40px -24px rgba(0,0,0,0.25)" }}>
            ...
            <div className="p-5 space-y-5">
              ...
              {/* Result — stacked analyses (newest first) — shared <AnalysisResultView> */}
              {detailData.result && (() => {
                ...
                return (
                  <AnalysisResultView
                    ...
                    activeTask={activeTasks[detailId!]}
                    followupText={followupText}
                    setFollowupText={setFollowupText}
                    followupSubmitting={followupSubmitting}
                    onStartFollowup={startFollowup}
                    onDeepAnalysis={handleDeepAnalysisWithDirection}
                    onCopy={copy}
                    renderEngineerFeedback={(r) => (
                      <EngineerLabelFeedbackWidget
                        ...
                        onSubmit={(actually_needed_engineer, note) => {
                          const me = (typeof window !== "undefined" ? localStorage.getItem("appllo_username") || "" : "");
                          return submitEngineerLabelFeedback({
                            ...
                            setIssueAnalyses((prev) => {
                              ...
                              return {
                                ...
                                [detailId!]: arr.map((a) =>
                                  a.task_id === r.task_id
                                    ? { ...a, engineer_label_feedback: resp.actually_needed_engineer, engineer_label_feedback_by: me, engineer_label_feedback_at: new Date().toISOString(), engineer_label_feedback_note: note || "" }
                                    : a
                                ),
                              };
                            });
